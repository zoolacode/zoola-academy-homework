...
function getIndex() {
    while(true){
        ...
        if (randomIndex !== newIndex) return newIndex;
    }
}
function immediateInterval(gameFunction, interval) {
    if (currentScore >= finalScore || currentScore <= -finalScore) stopGame(activeElement);
    else {
        gameFunction();
...
function changeColorScore() {
    if (currentScore > 0) scoreContainer.style.backgroundColor = "#419884";
    else scoreContainer.style.backgroundColor = "#cc4040";
}
