...
for(let i = 0; i < itemsQuantity; i++)appendItems();
...
function resetPrevGameResult() {
    if (highlightedElement) highlightedElement.className = "item";
...
function handleScoreColor() {
    if (+scoreContainer.innerText < 0) scoreContainer.classList.add("danger");
    else scoreContainer.classList.remove("danger");
}
function handleClick() {
    if (isClicked) scoreContainer.innerText = currentScore -= doubleClickPoint;
    else scoreContainer.innerText = currentScore += successClickPoint;
    handleScoreColor();
    isClicked = true;
...
function handleClickOutsideElement(element) {
    if (element.target.classList.value === "item") scoreContainer.innerText = currentScore -= clickOutsideElementPoint;
    if (currentScore <= -limitScore || currentScore >= limitScore) {
        createResultNotification(currentScore);
        stopGameIteration(highlightedElement);
    }
...
function runGameIteration() {
   ...
   */ if (deactivateElement) deactivateElement();
    ...
    if (!isClicked) scoreContainer.innerText = currentScore -= 5;
    handleScoreColor();
    if (activeElement) {
...
function createResultNotification(result) {
    ...
    if (result <= -limitScore) notification.innerText = "Oops! Try again!";
    else notification.innerText = "Congratulations! You win";
